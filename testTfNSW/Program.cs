...
namespace testTfNSW
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			double cx = xc - Math.Sin(thiRadians) * radius;
			double cy = yc + Math.Cos(thiRadians) * radius;
			...
			//IfcCartesianPoint point4 = new IfcCartesianPoint(db, 139.4879381, 9.0015170);
			...
			IfcCurveSegment curveSegment = new IfcCurveSegment(IfcTransitionCode.CONTINUOUS, new IfcAxis2Placement2D(point1), lineLength, xLine);
			compositeSegments.Add(curveSegment);
			...
			linearSegment.ObjectPlacement = new IfcLocalPlacement(alignmentHorizontal.ObjectPlacement, db.Factory.XYPlanePlacement);
			...
			curveSegment = new IfcCurveSegment(IfcTransitionCode.CONTINUOUS, db.Factory.Origin2dPlace, transitionLength, seriesParameterCurve);
			compositeSegments.Add(curveSegment);
			...
			IfcCircle circle = new IfcCircle(db.Factory.Origin2dPlace, radius);
			curveSegment = new IfcCurveSegment(IfcTransitionCode.CONTINUOUS, circlePlacement, arcLength, circle);
			compositeSegments.Add(curveSegment);
			...
			arcSegment.ObjectPlacement = new IfcLocalPlacement(alignmentHorizontal.ObjectPlacement, db.Factory.XYPlanePlacement);
			...
			alignmentHorizontal.Representation = new IfcProductDefinitionShape(new IfcShapeRepresentation(db.Factory.SubContext(IfcGeometricRepresentationSubContext.SubContextIdentifier.Axis), horizontalCurve, ShapeRepresentationType.Curve2D));
			...
			IfcLine gradientcurve = new IfcLine(db.Factory.Origin, new IfcVector(new IfcDirection(db, 0.99, 0.01, 0), 1)); //not right and should it be xy or xz
			IfcCurveSegment verticalCurveSegment = new IfcCurveSegment(IfcTransitionCode.CONTINUOUS, new IfcAxis2Placement3D(db.Factory.Origin2d), 50, gradientcurve);
